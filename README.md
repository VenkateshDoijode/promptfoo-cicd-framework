...
### Structure of a red team result

Similar shape, but each result also includes:
- **`pluginId`** — which plugin generated the attack (e.g., `pii:direct`)
- **`strategyId`** — which strategy wrapped/delivered it (e.g., `jailbreak`)
- **`success`** — here, `success: true` typically means **the target resisted the attack**; a failed grading means the attack **got through** — always check the grading semantics for red team results specifically, since it inverts intuitive pass/fail compared to quality evals.

### Pass rate

- **Quality eval** pass rate = `stats.successes / (stats.successes + stats.failures)`. This project gates CI at **80%** (`--fail-threshold 0.8`).
- **Red team scan** results are **not threshold-gated** in either pipeline (`allow_failure: true` in GitLab, no `--fail-threshold` in Jenkins) — findings are meant to be **reviewed manually**, since a single successful jailbreak may or may not represent an acceptable risk depending on severity.

---

## Troubleshooting Notes

- **System prompt path mismatch:** `promptfooconfig.yaml` and `promptfooconfig.fast.yaml` reference `file://prompts/system-prompt.txt` (plural `prompts/`), but the actual folder in this repo is `prompt/` (singular). Either rename the folder to `prompts/`, or update the `systemPrompt:` path in both configs to `file://prompt/system-prompt.txt` before running — otherwise Promptfoo will fail to load the system prompt.
- **Missing `OPENAI_API_KEY`** will cause both the eval and the `llm-rubric` judge grading to fail immediately — always confirm the variable is set/masked in your CI system before triggering a pipeline.
- **`LOAN_API_KEY`** is only required if you actually exercise the `https` "SecureBank Loan API" provider in `promptfooconfig.fast.yaml`; omit it if that provider isn't reachable from your environment yet.
